...
public class CompositConvierView extends LeyoutComponentView {

    private int RBoard;	//Внешний радиус доски
    private int RLine;	//Внешний радиус ленты
    private int LLine; 	//Длина ленты

    public CompositConvierView(LeyoutComponentController controller){
        super(controller);
    }
    ...
    public void paint() {
        ...
        ConvierController cc = ((CompositConvierController)controller()).getConvierController();
...
//        cc.setS(controller().S());
    ...
    public int countPos(){
        return ((CompositConvierController)controller()).countPos();
    }
}
